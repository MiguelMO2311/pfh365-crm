...
export default function ExpedienteDetail() {
  ...
  if (!expediente) {
    return <div className="p-10 text-center text-gray-500">Expediente no encontrado</div>;
  }

  const timelineEvents: TimelineEvent[] = [
    { id: 1, type: 'creation', title: 'Expediente Creado', description: 'Por API de Integración', date: new Date(expediente.created_at).toLocaleDateString() },
    { id: 2, type: 'document', title: 'Pendiente de Documentación', description: 'Esperando carga de FEIN/FIAE', date: 'Actual', isActive: true },
  ];
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-4 flex flex-col sm:flex-row sm:flex-wrap sm:space-x-8">
              <div className="flex items-center text-sm font-medium text-gray-600 bg-white/50 px-4 py-2 rounded-xl backdrop-blur-sm border border-gray-200/50">
                <span className="text-gray-400 mr-2">Cliente</span> {expediente.cliente_nombre} &middot; {expediente.cliente_email || 'Sin email'}
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card noPadding>
            ...
            <div className="p-2">
              {expediente.operaciones?.length > 0 ? (
                expediente.operaciones.map((op: any) => (
                  <div key={op.id} className="m-2 p-4 rounded-xl border border-gray-100 bg-white hover:border-corporate-200 hover:shadow-md transition-all group cursor-pointer flex items-center justify-between">
                     <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h4 className="font-bold text-gray-900">{op.tipo_operacion}</h4>
                          <p className="text-sm text-gray-500">{op.estado} &middot; Vinculada a Entidad Fiscal</p>
                        </div>
                     </div>
                ...
                <div className="p-6 text-center text-gray-500 text-sm">Sin operaciones enlazadas.</div>
              )}
            </div>
          </Card>
        ...
        <div className="col-span-1 space-y-6">
          <Card noPadding className="border-0 shadow-lg ring-1 ring-black/5">
            <div className="bg-gradient-to-b from-corporate-800 to-corporate-950 px-6 py-6 text-white text-center">
              ...
              <p className="text-corporate-200 text-sm mt-1">Requisitos Pendientes</p>
            </div>
            <div className="p-6 bg-white">
              <ul className="space-y-4">
                 <li className="flex items-start gap-3 group">
                   <div className="text-sm leading-6 flex-1 text-center text-gray-500 italic pb-4">
                     El checklist documental estricto se sincronizará automáticamente al subir documentos.
                   </div>
                 </li>
              </ul>
              ...
              <div className="mt-8">
                <button className="w-full btn btn-secondary flex justify-center gap-2 items-center bg-gray-50 hover:bg-gray-100 border-dashed border-2 border-gray-300">
                  ...
                  <span className="text-gray-600 font-semibold">Subir Documento PDF</span>
                </button>
              </div>
            </div>
          </Card>
